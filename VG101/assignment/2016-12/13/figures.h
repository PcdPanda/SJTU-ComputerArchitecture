...
{
	public:
		Quad(Vec p1={0.5,0.5},Vec p2={-0.5,0.5},Vec p3={-0.5,-0.5},Vec p4={0.5,-0.5},float R=0,float G=0,float B=0)
		{
			anchor={0,0};
			r=R;g=G;b=B;p[0]=p1;p[1]=p2;p[2]=p3;p[3]=p4;
			for(int i=0;i<=3;i++)anchor=anchor+p[i];
			anchor.x/=4;anchor.y/=4;
		}
		void draw()
...
{
	public:
		SemiCircle(Vec p1={0,0},Vec p2={-0.1,0},float rad=0.1,float R=0,float G=0,float B=0)
		{r=R;g=G;b=B;radius=rad;p[0]=anchor=p1;p[1]=p2;}
		void draw()
		{
			glColor3f(r,g,b);
			glBegin(GL_POLYGON);
			float x0=p[1].x-p[0].x,y0=p[1].y-p[0].y;
			float n=atan2(y0,x0);
			for(float i=n;i<=n+PI;i+=0.005)
			{glVertex2f(p[0].x+radius*cos(i),p[0].y+radius*sin(i));}
			glEnd();
		}
		void move(Vec v){anchor=anchor+v;p[0]=p[0]+v;p[1]=p[1]+v;}
		void rotate(float angle=PI/16){for(int i=0;i<=1;i++)p[i]=p[i].rotate(angle,anchor);}	
		void zoom(float scale){radius*=scale;}
	private:float radius;Vec p[2];
};
